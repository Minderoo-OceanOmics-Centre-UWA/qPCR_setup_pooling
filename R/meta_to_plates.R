...
meta_to_plates <- function(metadata,
                           ...
                           plate_height = 8,
                           controls = c("NTC", "ITC"),
                           control_pattern = "WC|FC|DI|EB|BC|NTC|ITC|Cont|BL",
                           skip_plates = list(),
                           skip_samples = list(),
                           strategy = "UC",
                           prefix = "") {
    ...
    if (length(metadata) == 1) {
        ...
        for (file in metadata) {
            file_split <- str_split(file, "_")[[1]]
            num <- file_split[length(file_split) - 1]
            project <- paste0("OcOm_", num)
            tmp_df  <- import_meta_df(file, run)
            tmp_df$project <- project
    ...
    if (length(metadata) == 1) {
        ...
        for (p in prefix) {
            if (nchar(p) > 0) {
                if (substring(p, nchar(p)) != "_") {
                    projects <- c(projects, p)
                    p <- paste0(p, "_") 
                    prefix_tmp <- c(prefix_tmp, p)
                }
            }
        }
        prefix <- prefix_tmp
    ...
    sam_colours <- c()
    i <- 0
    for (sample in unique(position_df$sample_id)) {
        i <- i + 1
        colour <- colours[i]
        sams <- c(sams, sample)
        sam_colours <- c(sam_colours, colour)
        if (i == 21) {
            i = 0
        }
    }
    ...
    for (assay in assays) {
        for (p in 1:plate_count[assay][[1]]) {
            if (p >= plates_to_skip[assay][[1]]){
                plate <- paste0("Plate", p)
                for (pref in prefix) {
                    QS7_outfile <- paste0(output_dir, pref, "QS7_plate_import_", assay, "_", plate, ".csv")
                    ...
                    write("* Barcode = ?", file = QS7_outfile, append = TRUE)
                    ...
                    filled_positions <- position_df[position_df$replicate != "pool" & position_df$assay == assay & position_df$plate_number == plate, "Pos"]
