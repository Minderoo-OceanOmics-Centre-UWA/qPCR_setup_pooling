...
# File name structure is used to determine assay and plate number and so much be YYYYMMDD_Expedition_Assay_Plate# 
# merge all relevant text files from all qPCR plates for a given assay - these will be separate files
# create a list of file names
exp_name <- "testing_code"
list.files("qPCR_data/")
...
read_data <- function(x) {
  ...
  data$fileName <- gsub(".txt", "", str_split(file, "\\/", simplify = TRUE)[,2])
...
sample_order <- function(x) order(mixedorder(x))
...
lc480_data_sample <- all_lc480_data %>%
  ...
  dplyr::select(-Position, -plate_number) %>% rename(c('assay'='ASSAY')) %>% 
...
# (for real samples only, criteria not applied to controls as these are all taken through to sequencing )
rep_failed <- lc480_data_sample %>%
 mutate(discard = case_when(replicate == "pool" ~ "NA",
                            sample_type == "sample" & EPF <= 3 ~ "DISCARD",
                             TRUE ~ "KEEP"))  %>%
  arrange(sample_order(sample_id))
rep_failed

# summary of number of reps to be discarded per sample
rep_failed_summary <- rep_failed %>%
    filter( replicate != "pool") %>% 
    dplyr::group_by(sample_id, ASSAY) %>%
...
epf_cal <- rep_failed %>%
  dplyr::group_by(ASSAY, sample_id, sample_type) %>%
  dplyr::summarise(median = median(EPF, na.rm = TRUE), mean = mean(EPF, na.rm = TRUE), sd = sd(EPF, na.rm = TRUE), min =  min(EPF), max = max(EPF), diff_epf = max(EPF) - min(EPF), number_valid_reps = sum(EPF >= 3)) %>%
 arrange(sample_order(sample_id)) %>% 
  ungroup()
...
epf_cal_mean <- epf_cal %>%
  filter(sample_type == "control" | (sample_type == "sample" & number_valid_reps >= 2)) %>%
  mutate(sample_replicate = paste0(sample_id,"-pool")) %>%
...
# fixed volume per plate won't work because of potential big differences in number of samples per plate
# using grouping of samples based on "minipool" approach (as used in the past)
# "minipools" created based on 0.5 EPF difference (this is assuming correct florescence values from LC480 where EPF = 15-20 )
# number of actual samples and controls per plate

minipool_overview <- position_df_pool %>%
  group_by(ASSAY, plate_number, sample_type) %>%
  dplyr::summarise(count_samples = n_distinct(sample_id), min =  min(mean), max = max(mean), diff_mean = max(mean) - min(mean), n_groups = round((max(mean) - min(mean))/0.5, digits = 0))
minipool_overview
...
# dplyr::summarise(min =  min(mean), max = max(mean), diff_mean = max(mean) - min(mean), n_groups = round((max(mean) - min(mean))/0.5, digits = 0))
# minipool_overview
...
for (assay in unique(position_df_pool$ASSAY)) {
  subs <- position_df_pool |> filter(ASSAY == assay)
  for (plate in unique(subs$plate_number)) {
    plate_num  <- plate_num + 1
    plate_keys <- c()
    for (sam_type in unique(subs$sample_type)) {
      curr_key   <- paste0(assay, "_", plate, "_", sam_type)
      ...
      # add volume to pool as well as info on tube destination (Plate 1 = P16-A1 well/tube = samples and P16-B1 well/tube = controls, Plate 2 = P16-A2 well/tube = samples and P16-B2 well/tube = controls,)
      ...
      minipool_calc_vols[curr_key] <- minipool_calcs[curr_key][[1]] %>%
        left_join(., volume_ranges[curr_key][[1]], by = "miniPool_id") %>%
        mutate(DestinationPosition = "P16", DestinationWell = dwell) %>%
        data.frame() %>%
        list()
      
      # volume_sums  <- append(volume_sums, sum(minipool_calc_vols[curr_key][[1]]$vol_ul))
      # volume_beads <- append(volume_beads, (1.8 * sum(minipool_calc_vols[curr_key][[1]]$vol_ul)))
      
      
      # check "minipools" in a plot to confirm group/volume allocation 
      minipool_plots[[curr_key]] <- ggplot(minipool_calc_vols[curr_key][[1]], aes(x = sample_replicate, y = mean)) +
  ...
  rbind(minipool_calc_vols[plate_keys[1]][[1]], minipool_calc_vols[plate_keys[2]][[1]]) %>%
     mutate(SourcePosition = sourcepos) %>%
      dplyr::select(ASSAY, SourcePosition, SourceWell = Pos, Volume = vol_ul, DestinationPosition, DestinationWell) %>%
      arrange(sample_order(SourceWell)) %>%
     write_csv(paste0("biomek_pooling_workbook_", assay, plate,".csv"))
  }
