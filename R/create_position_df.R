        ...
        for (plate_num in (1:plate_count)) {
            ...
            for (i in 1:((plate_height * 2) * (plate_width * 2))) {
                ...
                position_df[pos_row, "sample_replicate"] <- paste0(
                    curr_sample, "-", replicate
                )
                position_df[pos_row, "plate_number"] <- paste0(
                    "Plate", plate_num
                )
                position_df[pos_row, "assay_plate_number_Pos"] <- paste0(
                    assay, ".", "Plate", plate_num, ".", well
                )
                position_df[pos_row, "plate_number_Pos"] <- paste0(
                    "Plate", plate_num, ".", well
                )
                position_df[pos_row, "sample_type"] <- sample_type

                # If the current replicate is '1',
                # we need to move one column to the right
                # and stay in the same row
                if (replicate == "1") {
                    replicate <- "2"
                    well_col  <- well_col + 1

                # If the current replicate is '2',
                # we need to move one column to the left and one row down
                } else if (replicate == "2") {
                    replicate      <- "3"
                    well_row_index <- well_row_index + 1
                    well_col       <- well_col - 1

                # If the current replicate is '3',
                # we need to move one column to the right
                } else if (replicate == "3") {
                    replicate <- "pool"
                    well_col  <- well_col + 1
